...
namespace VRStandardAssets.Menu
{
    ...
    public class MenuItemPopout : MonoBehaviour
    {
        ...
        public Vector3 m_TargetPosition;                       // The current position being aimed for.
        ...
        private void Start ()
        {
			...
			m_PoppedPosition = m_Transform.position - (m_Transform.position - Vector3.up * m_Transform.position.y).normalized * m_PopDistance;
        }
		...
		private void HandleOver()
		{
			...
			m_PoppedPosition = transform.position - (m_Transform.position - Vector3.up * m_Transform.position.y).normalized * m_PopDistance;
			m_StartPosition = transform.position;
			m_Transform.position = m_PoppedPosition;
